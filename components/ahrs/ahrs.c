...
void ahrs_update(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz)
{
  ...
  // Rate of change[变化率/导数] of quaternion[四元数] from gyroscope
  ...
  if (!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f)))
  {
    ...
    // Normalise accelerometer measurement [归一化 加速度 数据]
    recipNorm = 1.0 / sqrt(ax * ax + ay * ay + az * az);
    ax *= recipNorm;
    ay *= recipNorm;
    az *= recipNorm;

    // Normalise magnetometer measurement [归一化磁力计数据]
    recipNorm = 1.0 / sqrt(mx * mx + my * my + mz * mz);
    mx *= recipNorm;
    my *= recipNorm;
    mz *= recipNorm;

    // Auxiliary[辅助] variables to avoid[避免] repeated arithmetic[计算]
    _2q0mx = 2.0f * q0 * mx;
    _2q0my = 2.0f * q0 * my;
    _2q0mz = 2.0f * q0 * mz;
    _2q1mx = 2.0f * q1 * mx;
    ...
    // Reference direction[参考方向] of Earth's magnetic[磁] field
    ...
    // Gradient[梯度] decent algorithm[梯度下降方法] corrective[矫正] step
  ...
  q0 += qDot1 * (1.0f / sampleFreq);  // 导数*delta_time==>角度变化的积分===>角度变化
  q1 += qDot2 * (1.0f / sampleFreq);
  q2 += qDot3 * (1.0f / sampleFreq);
  q3 += qDot4 * (1.0f / sampleFreq);

  // Normalise quaternion[保证四元数的合法性]
